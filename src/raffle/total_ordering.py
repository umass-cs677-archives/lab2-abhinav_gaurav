...
class TotalOrdering:
    ...
    def multicastAck(self, logical_id, pid, server_pid):
        ...
        if len(elem_set) == self.greater_pids[int(pid)]:
            for pid in self.pids:
                if pid <= self.pid:
                    r = requests.get('http://' + self.pid2server(pid) + '/receiveMulticastAck/%d/%d' % (
                    int(logical_id), int(pid)))  # TODO create dictionary pid2server
    ...
    def send_acknowledgement(self, logical_id, pid):
        ...
        for server in self.servers:
            r = requests.get('http://' + server + '/multicastAck/%d/%d/%s' % (int(logical_id), int(pid),
                                                                              "127.0.0.1:%d" % (
